...
Ext.define('Ext.calendar.App', {
    ...
    constructor : function() {
        ...
        //load job 
        var jobs=Ext.calendar.data.Events.getData().evts;
        window.jobContainer={
            add:function(jobdata){
                var jtime= new Date(jobdata.start);
                window.jobContainer[jobdata.id]=(function(job,jt){
                    var jobentity = schedule.scheduleJob(jt.getTime(), function(){
                        nwNotify.notify({
                            title: job.title,
                            text: job.notes,
                            displayTime: 10000,
                            // Alternate sound_1.wav and sound_2.wav
                            sound: 'F:/00work/04share/calendar/sounds/sound_'+((num % 2) +1) +'.wav',
            ...
            ,drop:function(id){
                window.jobContainer[id].cancel();
            }
        };
        var jtime ;
        for(var i=0;i<jobs.length;i++){
            window.jobContainer.add(jobs[i]);
        }
    ...
    mapdeal:function(item){
        var newd={};
        for(var pn in Ext.calendar.data.EventMappings){
            if(item.data[pn]){
                if(Ext.calendar.data.EventMappings[pn].mapping=="id"){
                    newd[Ext.calendar.data.EventMappings[pn].mapping]=new Date().getTime();
                }
                else if(Ext.calendar.data.EventMappings[pn].mapping=="start"){
                    newd[Ext.calendar.data.EventMappings[pn].mapping]=item.data[pn].Format("yyyy-MM-dd hh:mm:ss");
                }
                else if(Ext.calendar.data.EventMappings[pn].mapping=="end"){
                    newd[Ext.calendar.data.EventMappings[pn].mapping]=item.data[pn].Format("yyyy-MM-dd hh:mm:ss");
                }
                
                else{
                    newd[Ext.calendar.data.EventMappings[pn].mapping]=item.data[pn];
                }
            }
        }
        return newd;
    ...
    saveEvent:function(){
        ...
        var items = this.eventStore.data.items;
        var newDatas=[];
        for(var i=0;i<items.length;i++){
            newDatas.push(this.mapdeal(items[i]));
        }
        fs.writeFile('alert.json',JSON.stringify(newDatas));
    },
                ...
                listeners: {
                    'eventadd': {
                        fn: function(win, rec){
                            ...
                            me.saveEvent();
                            window.jobContainer.add(this.mapdeal(rec));
                            this.showMsg('Event '+ rec.data.Title +' was added');
                        },
                        scope: this
                    },
                    'eventupdate': {
                        fn: function(win, rec){
                            ...
                            this.eventStore.sync();
                            me.saveEvent();
                            window.jobContainer.drop(rec.id);
                            window.jobContainer.add(this.mapdeal(rec));
                            this.showMsg('Event '+ rec.data.Title +' was updated');
                        },
                        scope: this
